...
export default function CompeticionDetalle({ evento }: { evento: any }) {
	...
	return (
		<>
			<Head>
				<title>{evento?.detalles?.tituloEvento}</title>
				<meta property="og:title" content={evento?.detalles?.tituloEvento} />
				<meta
					property="og:description"
					content={`Detalles de: ${evento?.detalles?.tituloEvento}`}
				/>
				<meta
					property="og:image"
					content={evento?.detalles?.imagen?.node?.sourceUrl}
				/>
				<meta
					property="og:url"
					content={`https://badflay.com/competicion/${evento?.slug}`}
				/>
